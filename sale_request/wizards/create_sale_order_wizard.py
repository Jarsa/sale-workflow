...
class CreateSaleOrderWizard(models.TransientModel):
    ...
    confirm_sale_order = fields.Boolean(
        help='If you check this box the new sale order will be'
    ...
    @api.model
    def prepare_sale_order(self, line):
        return {
            'partner_id': line.request_id.partner_id.id,
    ...
    def create_sale_order(self):
        ...
        request_line = self.request_line_id
        order = so_obj.create(self.prepare_sale_order(request_line))
        ...
        for item in items:
            ...
            values = self.prepare_sale_order_line(order, item)
            sol_obj.create(values)
        if self.confirm_sale_order:
            ...
            if new_remaining_qty < 0:
                ...
                return {
                    'warning': {
                        'title': _('Error!'),
                        'message': _('You cannot request more than the '
                                     'remaining qty of this base sale order.'),
                    },
                }
            self.remaining_product_qty = new_remaining_qty
